...
then
  ...
  then
    ...
    ILYDE_JUPYTER_ROOT_BROWSING_SUPPORTED="false"
  else
    echo "Running an IPython version more recent than 4.1."
    ILYDE_JUPYTER_ROOT_BROWSING_SUPPORTED="true"
  fi
else
...
if [ "$ILYDE_JUPYTER_ROOT_BROWSING_SUPPORTED" == "true" ]
then
  ...
  echo "c.NotebookApp.default_url = '/tree${ILYDE_WORKING_DIR}'" >> "${CONF_FILE}"
else
  ...
  MAIN_PROJECT_REDIRECT_DIR=${ILYDE_WORKING_DIR#"/mnt"}
